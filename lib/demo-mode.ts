...
import grantsData from '@/data/mock/grants.json'
import focusAreasData from '@/data/mock/focus-areas.json'
...
const mockDataMap: Record<string, any> = {
  ...
  'grants.json': grantsData,
  'focus-areas.json': focusAreasData,
...
export function handleMockQuery(body: string): any {
  try {
    ...
    if (query.includes('GetGrants') || query.includes('nodeGrants')) {
      return loadMockData('grants.json')
    }

    if (query.includes('GetFocusAreas') || query.includes('nodeFocusAreas')) {
      return loadMockData('focus-areas.json')
    }
