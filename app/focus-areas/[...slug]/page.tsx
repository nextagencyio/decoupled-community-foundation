...
import { GET_FOCUS_AREA_BY_PATH } from '@/lib/queries'
import { DrupalFocusArea } from '@/lib/types'
...
interface FocusAreaByPathData {
  route: {
    entity: DrupalFocusArea
  } | null
}
...
async function getFocusArea(path: string): Promise<DrupalFocusArea | null> {
  try {
    ...
    const { data } = await apolloClient.query<FocusAreaByPathData>({
      query: GET_FOCUS_AREA_BY_PATH,
      variables: { path },
...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  const path = `/focus-areas/${slug.join('/')}`
  const item = await getFocusArea(path)

  if (!item) {
    return { title: 'Focus Area Not Found | Community Foundation' }
  }

  return {
    title: `${item.title} | Community Foundation`,
            ...
            <div className="lg:col-span-1">
              <div className="bg-white rounded-xl shadow-sm p-6 sticky top-24">
                ...
                <dl className="space-y-4">
                  ...
                  {(item as any).investmentTotal && (
                    <div>
                      <dt className="text-sm text-gray-500">Total Investment</dt>
                      <dd className="font-semibold text-gray-900">{(item as any).investmentTotal}</dd>
                    </div>
                  )}
                  {(item as any).grantsAwarded && (
                    <div>
                      <dt className="text-sm text-gray-500">Grants Awarded</dt>
                      <dd className="font-semibold text-gray-900">{(item as any).grantsAwarded}</dd>
                    </div>
                  )}
                </dl>
                <div className="mt-8">
