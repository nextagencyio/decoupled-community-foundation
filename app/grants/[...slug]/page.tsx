...
import { GET_GRANT_BY_PATH } from '@/lib/queries'
...
interface GrantByPathData {
  route: {
    entity: DrupalGrant
  } | null
}

async function getGrant(path: string): Promise<DrupalGrant | null> {
  try {
    const requestHeaders = await headers()
    const apolloClient = getServerApolloClient(requestHeaders)
    const { data } = await apolloClient.query<GrantByPathData>({
      query: GET_GRANT_BY_PATH,
      variables: { path },
...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  const path = `/grants/${slug.join('/')}`
  const item = await getGrant(path)

  if (!item) {
    return { title: 'Grant Not Found | Community Foundation' }
  }
            ...
            <div className="lg:col-span-1">
              <div className="bg-white rounded-xl shadow-sm p-6 sticky top-24">
                ...
                <dl className="space-y-4">
                  {(item as any).fundingAmount && (
                    <div>
                      <dt className="text-sm text-gray-500">Funding Amount</dt>
                      <dd className="font-semibold text-gray-900">{(item as any).fundingAmount}</dd>
                    </div>
                  )}
                  {(item as any).applicationUrl && (
                    <div>
                      <dt className="text-sm text-gray-500">Application URL</dt>
                      <dd className="font-semibold text-gray-900">{(item as any).applicationUrl}</dd>
                    </div>
                  )}
                </dl>
                <div className="mt-8">
