...
interface FocusAreaCardProps {
  item: DrupalFocusArea
}

export default function FocusAreaCard({ item }: FocusAreaCardProps) {
  return (
    <Link
      href={item.path || '#'}
    ...
    >
      ...
      <div className="flex-1 p-6 flex flex-col justify-center">
        {(item as any).iconName && (
          <p className="text-sm text-primary-600 font-medium mb-1">{(item as any).iconName}</p>
        )}
